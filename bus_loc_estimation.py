...
# # Função para calcular a localização estimada do ônibus
# # Função para calcular a localização estimada do ônibus
...
#     if total_weight == 0:
#         print("⚠️ Nenhum peso válido encontrado. Não é possível estimar a localização do ônibus.")
#         return None, None  # Ou pode retornar valores padrão, como (0, 0)

#     # Calcular a localização média ponderada
#     estimated_lat = total_lat / total_weight
#     estimated_lon = total_lon / total_weight

#     return estimated_lat, estimated_lon


# # Função para combinar as duas
...
#     # Passo 1: Calcular os pesos para cada usuário
#     for user in users_vectors:
#         user['weight'] = calc_raw_weight(user, total_samples)  # Calcula o peso e adiciona ao dicionário
#         print(f"User {user['user_id']} raw weight: {user['weight']}")
    
#     # Passo 2: Estimar a localização do ônibus usando os pesos calculados
